                ...
                if (Ҡ != ҍ.Д) {
                    ...
                    if (μ) {
                        if (ù()) {
                            ë(σ, υ, τ, 0.25f, true);
                            Ĕ();
                            ώ = "Aligning to planet: " + Math.Round(ė - 0.25f, 2) + "°";
                            if (ě)
                                α(true, true);
                        } else
                            α(true, true);
...
String ϣ(bool ƛ) {
    ...
    if (Є == ϸ.Ϸ) {
        ...
        if (ύ(ref Ι, ƴ, A++, ƛ," Depth mode: " + ϝ(ʀ))) {
            ʀ = ͼ(ʀ);
        }

        if (ύ(ref Ι, ƴ, A++, ƛ, " Start pos: " + ϝ(ʄ))) {
            ʄ = ͼ(ʄ);
        }
        ...
        Ɨ += " Leave connector 1:\n";
        if (ύ(ref Ɨ, ƴ, A++, ƛ, " - " + ϓ(ʔ.Ų)))
            ʔ.Ų = ͼ(ʔ.Ų);

        if (!ʔ.Ǝ())
            Ɨ += "\n";
        else if (ύ(ref Ɨ, ƴ, A++, ƛ, " - Delay: " + Ǖ((int)ʔ.Ƅ)))
            ʔ.Ƅ = ʞ(ʔ.Ƅ, ʜ);

        Ɨ += " Leave connector 2:\n";
        if (ύ(ref Ɨ, ƴ, A++, ƛ, " - " + ϓ(ʒ.Ų)))
            ʒ.Ų = ͼ(ʒ.Ų);
        ...
        if (ύ(ref Ɨ, ƴ, A++, ƛ, " = " + (ʔ.ƕ != "" ? ʔ.ƕ : "-")))
            ʔ.ƕ = ʙ(ref ϧ);

        Ɨ += " Timer: \"Docking connector 2\":\n";
        if (ύ(ref Ɨ, ƴ, A++, ƛ, " = " + (ʒ.Ɣ != "" ? ʒ.Ɣ : "-")))
            ʒ.Ɣ = ʙ(ref Ϩ);
        ...
        if (ύ(ref Ι, ƴ, A++, ƛ, " Ejection: " + Ϙ(ɻ))) {
            ɻ = ͼ(ɻ);
        }

        if (ύ(ref Ι, ƴ, A++, ƛ, " Toggle sorters: " + (ɽ ? "On" : "Off"))) {
            ɽ = !ɽ;
            if (ɽ)
                Ũ(ũ);
        }

        if (ύ(ref Ι, ƴ, A++, ƛ, " Unload ice: " + (ɺ ? "On" : "Off")))
            ɺ = !ɺ;
        ...
        if (ύ(ref Ι, ƴ, A++, ƛ, " On damage: " + Ϛ(ʆ))) {
            ʆ = ͼ(ʆ);
        }
        ...
        if (ύ(ref Ɨ, ƴ, A++, ƛ, " On damage: " + Ϛ(ʆ))) {
            ʆ = ͼ(ʆ);
        }
...
void ʘ(string Ô) {
    ...
    IMyTerminalBlock q = χ.GetBlockWithName(Ô);
    if (q == null || !(q is IMyTimerBlock)) {
        ώ = "Timerblock " + Ô + " not found!";
        return;
    } 
...
Vector3 Ҽ(int Y, int X, bool һ) {
    ...
    float Ă = ((Ӂ - 1f) / 2f) - Y;
    float K = ((Ӏ - 1f) / 2f) - X; ҿ = Ǟ.ɉ + Ǟ.ĕ * Ă * Й + ӄ * -1 * K * Я;
    ҽ = true;
    return ҿ;
}
...
void Ұ() {
    ...
    Ǟ.ç = σ;
    Ǟ.ĕ = τ;
    Ӆ = М.WorldMatrix.Forward;
    ӄ = Ǟ.ç;
    if (Ӆ == Э.WorldMatrix.Down)
...
ӳ Ӯ(int ӭ, bool Ç) {
    ...
    if (ʄ == ʲ.ʱ) {
        ...
        int ӫ = (int)Math.Ceiling(Ӂ / 2f);
        int Ӫ = (int)Math.Ceiling(Ӏ / 2f);
        int ӽ = (int)Math.Floor(Ӂ / 2f);
        int ӿ = (int)Math.Floor(Ӏ / 2f);
        int Ԕ = 0;
...
void ӗ() {
    ...
    if (Ҡ == ҍ.Ҍ) {
        ...
        if (Vector3.Distance(Ӗ, ɉ) > 0.3f) {
            Vector3 Ӕ = ԍ + Ӆ * (ԁ + 0.1f);
            ñ(Ӕ, 4);
            ӕ = true;
        } else {
            float κ = Е(true);
            Vector3 ӓ = Һ(ԍ, Math.Max(ʁ + 1, ԁ + 1));
    ...
    if (Ҡ == ҍ.ҥ || Ҡ == ҍ.Ҥ) {
        ...
        if (Ԅ >= 0) {
            ...
            Vector3 Ӝ = Ҽ(ԓ, Ԓ, true) + Ȏ(Ӆ, ӄ * -1, new Vector3(Y, X, 0));
            ñ(Ӝ, 0.3f);
...
void З(bool Ç) {
    ...
    float ņ = Ӂ * Ӏ * Math.Max(1, ʁ);
    ӎ = Math.Max(ӎ, (float)Math.Min(ʝ / ņ * 100.0, 100));
}
...
void Ѹ() {
    ...
    χ.GetBlocksOfType(ҁ, Л);
    χ.GetBlocksOfType(П, Л);
    χ.GetBlocksOfType(Ҋ, Л);
    χ.SearchBlocksOfName(pamTag.Substring(0, pamTag.Length - 1) + ":", ƣ, q => q.CubeGrid == Me.CubeGrid && q is IMyTextSurfaceProvider);
    П = Ѧ(П, pamTag, true);
    э();
    ѵ(ƣ);
...
void Ҁ() {
    χ.GetBlocksOfType(Ч, Л);
    χ.GetBlocksOfType(Ъ, Л);
    χ.GetBlocksOfType(Δ, Л);
    χ.GetBlocksOfType(Щ, Л);
    χ.GetBlocksOfType(Р, Л);
    χ.GetBlocksOfType(Ц, Л);
    χ.GetBlocksOfType(Ф, q => q.CubeGrid == Me.CubeGrid && q.BlockDefinition.ToString().ToUpper().Contains("HYDROGEN"));
    χ.GetBlocksOfType(Х, Л);
...
void Ѻ() {
    χ.GetBlocksOfType(С, q => q.CubeGrid == Me.CubeGrid && q.InventoryCount > 0);
    Т.Clear();
...
bool ů() {
    ...
    χ.GetBlocksOfType(Ī, q => q.CubeGrid == Me.CubeGrid);
    for (int A = 0; A < Ī.Count; A++)
...
void Ş(bool ŝ, bool Ŝ, float ś, float Š) {
    ...
    Ь.Enabled = true;
    Ь.ShowOnHUD = ŝ;
    Ь.LeftExtend = (Ŝ ? 1 : ʃ) / 2f * Й - ŗ.X;
    Ь.RightExtend = (Ŝ ? 1 : ʃ) / 2f * Й + ŗ.X;
    Ь.TopExtend = (Ŝ ? 1 : ʂ) / 2f * Я + ŗ.Y;
    Ь.BottomExtend = (Ŝ ? 1 : ʂ) / 2f * Я - ŗ.Y;
    Ь.FrontExtend = (ŝ ? ʁ : ś) - ŗ.Z;
